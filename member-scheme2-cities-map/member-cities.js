...
function getDailyCityImage(city) {
  if (city.images && city.images.length) {
    ...
    return city.images[day % city.images.length];
  }
  return city.img;
}
...
cities.forEach((c, i) => {
  ...
  s.style.left = `${i * 100}%`;
  s.style.backgroundImage = `url('${getDailyCityImage(c)}')`;
  ...
  s.innerHTML = c.live
    ...
    : `<div class="city-letter">${c.letter || c.name[0]}</div><div class="city-word">${c.name}</div>`;
...
document.addEventListener("click", (e) => {
  if (e.target.id === "slideScrollBtn") scrollMap();
});
...
document.addEventListener("wheel", (e) => {
  ...
  if (e.deltaY > 0) {
    cur === cities.length - 1 ? scrollMap() : go(cur + 1);
  } else {
    go(cur - 1);
  }
}, { passive: true });
...
document.querySelector(".city-cont")?.addEventListener("touchend", (e) => {
  ...
  if (Math.abs(dx) > 55) {
    dx < 0 ? document.getElementById("rightBtn").click() : document.getElementById("leftBtn").click();
  }
}, { passive: true });
...
async function loadUpdates(range = "24h") {
  try {
    ...
    renderUpdates(
      getLocalUpdates().filter(u => now - new Date(u.createdAt).getTime() <= maxAge)
    );
  }
}
...
document.getElementById("updateForm").onsubmit = async (e) => {
  ...
  const body = {
    ...
    visibility,
    location,
    city: location ? "Around my live area" : "Around me",
    status: "pending/local",
    createdAt: new Date().toISOString()
  };

  try {
    const r = await fetch("/api/live-updates", {
      method: "POST",
