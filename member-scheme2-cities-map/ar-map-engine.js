...
(() => {
  ...
  const state = {
    ...
    watchId: null,
    stepMeters: 12,
    avatarUrl: "../assets/avatar/default-avatar.svg"
  };

  function fmt(n, d = 6) { return Number(n).toFixed(d); }
  function mapsUrl(lat, lng, z = 19) { return `https://maps.google.com/maps?q=${lat},${lng}&z=${z}&output=embed`; }
  function openMapsUrl(lat, lng) { return `https://www.google.com/maps/@${lat},${lng},19z`; }
  ...
  function approxLocation(loc) {
    if (!loc) return null;
    return { lat: Number(loc.lat).toFixed(4), lng: Number(loc.lng).toFixed(4), accuracy: Math.round(Number(loc.accuracy || 0)) };
  }
  function exactLocation(loc) {
    if (!loc) return null;
    return { lat: Number(loc.lat), lng: Number(loc.lng), accuracy: Math.round(Number(loc.accuracy || 0)), heading: loc.heading ?? null, speed: loc.speed ?? null, timestamp: loc.timestamp || Date.now() };
  }
  ...
  function render() {
    ...
    els.coords.innerHTML = `
      <b>Exact private POV:</b> ${fmt(lat)}, ${fmt(lng)} · accuracy ~${Math.round(accuracy || 0)}m<br>
      <b>Heading:</b> ${Math.round(state.heading)}° · <b>Move:</b> WASD / arrows / touch buttons<br>
      <b>Public share:</b> ${Number(lat).toFixed(4)}, ${Number(lng).toFixed(4)} approximate
    `;
    els.out.innerHTML = `
      <b>Real GPS locked:</b> ${fmt(lat)}, ${fmt(lng)} · accuracy ~${Math.round(accuracy || 0)}m<br>
      <b>Private:</b> exact coordinates stay in your session/user data. Public posts use rounded area by default.
    `;
  ...
  function setLocation(coords, fromWatch = false) {
    const loc = exactLocation({ lat: coords.latitude, lng: coords.longitude, accuracy: coords.accuracy, heading: coords.heading, speed: coords.speed, timestamp: Date.now() });
    state.real = loc;
    state.pov = { ...loc };
  ...
  function requestLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => { els.out.textContent = "Location blocked or unavailable: " + err.message; els.gps.textContent = "GPS blocked"; },
  ...
  function watchLocation() {
    ...
    state.watchId = navigator.geolocation.watchPosition(
      ...
      (err) => { els.out.textContent = "Live tracking error: " + err.message; },
  ...
  function move(dir) {
    ...
    const step = state.stepMeters;
    let north = 0, east = 0;
    if (dir === "forward") north = step;
    if (dir === "back") north = -step;
    if (dir === "left") east = -step;
    if (dir === "right") east = step;
  ...
  async function savePrivateLocation() {
    if (!state.real) return requestLocation();
    try {
      const r = await fetch("/api/location/current", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ location: state.real, pov: state.pov, heading: state.heading }) });
      const d = await r.json();
      if (!d.ok) throw new Error(d.error || "Save failed");
      els.out.innerHTML += `<br><b>Saved:</b> private location session saved at ${new Date(d.location.createdAt).toLocaleTimeString()}.`;
    } catch (error) {
  ...
  async function uploadAvatar(e) {
    ...
    if (file.type.startsWith("image/")) els.avatarPreview.src = localUrl;
    els.avatarOutput.textContent = "Uploading avatar...";
    ...
    try {
      ...
      if (d.mimeType?.startsWith("image/")) els.avatarPreview.src = d.avatarUrl;
      els.avatarOutput.textContent = `Avatar saved: ${d.avatarUrl}`;
    } catch (error) {
      els.avatarOutput.textContent = `Local preview only. Login/server upload needed: ${error.message}`;
    }
  }

  function bindHold(button, fn) {
    let t = null;
    const start = (e) => { e.preventDefault(); fn(); t = setInterval(fn, 260); };
  ...
  document.querySelectorAll("[data-move]").forEach(btn => bindHold(btn, () => move(btn.dataset.move)));
  document.querySelectorAll("[data-turn]").forEach(btn => bindHold(btn, () => turn(btn.dataset.turn)));
