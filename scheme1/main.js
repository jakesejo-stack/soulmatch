'use strict';
    ...
    function runSelfTests() {
      ...
      console.assert(Number.isFinite(normal.boundingSphere.radius), 'normal geometry radius should be finite');
      normal.dispose();

      const zeroFallback = createSafePlaneGeometry(0, 0);
      console.assert(Number.isFinite(zeroFallback.boundingSphere.radius), 'zero fallback geometry radius should be finite');
      zeroFallback.dispose();

      const nanFallback = createSafePlaneGeometry(Number.NaN, Number.NaN);
      console.assert(Number.isFinite(nanFallback.boundingSphere.radius), 'NaN fallback geometry radius should be finite');
      nanFallback.dispose();
    }
    ...
    document.querySelectorAll('.color-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (scheme === 3) {
          window.location.href = '../scheme3/';
          return;
        }
        if (scheme === 4) {
          window.location.href = '../scheme4/';
          return;
        }
        if (scheme === 5) {
